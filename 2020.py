...
def better_find_sum(sum_to_find):
    ...
    for first_Expense in expenses:
        for second_expense in expenses[expenses.index(last_item):expenses.index(first_Expense):-1]:
            number_of_checks = number_of_checks + 1
            last_item = second_expense
            sum = first_Expense + second_expense
            ...
            if sum == sum_to_find:
                return [first_Expense, second_expense], number_of_checks

            
if __name__ == "__main__":
    expenses.sort()    
    result,number_of_checks = find_sum(3030)
    print("numbers are {} {}, {} checks needed".format(result[0],result[1],number_of_checks))

    result,number_of_checks = better_find_sum(3030)
    print("numbers are {} {}, {} checks needed".format(result[0],result[1],number_of_checks))
